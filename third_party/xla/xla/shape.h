...
class Shape {
 public:
  ...
  PrimitiveType element_type() const {
    ...
    return (r == nullptr ? PRIMITIVE_TYPE_INVALID : r->element_type_);
  }
 ...
 private:
  ...
  static void Ref(Rep* r) {
    ...
    DCHECK_GT(r->ref.load(std::memory_order_relaxed), 0);
    r->ref.fetch_add(1, std::memory_order_relaxed);
  }
  ...
  static void Unref(Rep* r) {
    ...
    DCHECK_GT(r->ref.load(std::memory_order_relaxed), 0);
    if (r->ref.fetch_sub(1, std::memory_order_seq_cst) == 1) {
  ...
  State* mutable_state() {
    ...
    if (r == nullptr) {
      ...
      if (r->ref.load(std::memory_order_acquire) != 1) {
        // Remove sharing
        Rep* copy = new Rep;
        copy->element_type_ = r->element_type_;
        copy->state_ = r->state_;
    ...
    return &rep_do_not_use_directly_->state_;
  }
